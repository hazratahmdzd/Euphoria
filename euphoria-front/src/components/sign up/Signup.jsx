...
const Signup = () => {
    ...
    const validationSchema = Yup.object().shape({
        email: Yup.string().email(`${t("Error message")}`).required(`${t("Please enter your email")}`),
        password: Yup.string()
        ...
        <div className={signupStyle.container}>
            ...
            <form onSubmit={formik.handleSubmit} >
                    ...
                    <div className={signupStyle.email}>
                        <p>{t("Email address")}</p>
                        <input type="text" name="email" placeholder="designer@gmail.com" onChange={formik.handleChange} /><br />
                    ...
                    <div className={signupStyle.password}>
                        <div className={signupStyle.passwordText}>
                            <p>{t("Password")}</p>
                            {inputType === 'password' ? <p style={{ cursor: "pointer",marginRight:"70px" }} onClick={toggleInputType}>
                                <i className="fa-regular fa-eye"></i> {t("Show")}</p> :
                                 <p style={{ cursor: "pointer",marginRight:"70px"}} onClick={toggleInputType}>
